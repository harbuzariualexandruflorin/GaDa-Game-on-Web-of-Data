...
export async function getDetails(btn){
  ...
  result.push(`<img src="${cardInfo.avatar}" class="card-img-top mx-auto d-block rounded" style='width:40%' alt="Avatar" >`);
  ...
  */
  for(var prop in cardInfo) {
    ...
    result.push(`
      <div class="row">
        <div class="col-md-2"><b>${prop.charAt(0).toUpperCase() + prop.slice(1)}</b></div>
...
function objectToString(obj) {
  ...
  if (typeof obj != "object") {
    ...
    return `<li class="mt-0 mb-0">${String(obj).charAt(0).toUpperCase() + String(obj).slice(1)}</li>`;
  }
...
function renderTable() {
  ...
  fetch(`${API_URL}gada_card_deck/generate?` + new URLSearchParams({
